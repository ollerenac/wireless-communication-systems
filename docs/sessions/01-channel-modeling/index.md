...
**Panel izquierdo — $f_R(r)$, distribución de la envolvente**: el eje horizontal es la amplitud recibida $r$. La curva tiene su pico cerca del origen pero no en cero: hay una probabilidad no despreciable de que $r$ caiga en valores muy bajos. La **zona roja** marca el umbral de amplitud por debajo del cual la detección falla — su área es directamente la probabilidad de deep fade, es decir, la fracción del tiempo en que el canal produce errores con independencia del nivel medio de señal.
