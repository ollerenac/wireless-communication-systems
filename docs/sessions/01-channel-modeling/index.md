...
La forma de esta integral deja ver su significado: para cada valor posible de $\gamma$, se calcula la BER que produciría ($Q(\sqrt{\gamma})$) y se pondera por la probabilidad de que $\gamma$ tome ese valor ($f(\gamma)\,d\gamma$). El resultado es la BER promedio sobre todas las realizaciones del canal. Para evaluar esta integral hay que conocer $f(\gamma)$ — y esa distribución depende de cómo se distribuye la amplitud recibida, que es lo que los modelos de Rayleigh y Rician proporcionan.
