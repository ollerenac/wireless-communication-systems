...
Las Sesiones 01 y 02 construyeron los dos pilares del problema de transmisión digital: el canal inalámbrico y la modulación. La Sesión 01 mostró que los canales de banda ancha son frequency-selective — distintas frecuencias experimentan ganancias distintas, y los ecos producen ISI cuando el período de símbolo es menor que el delay spread. La Sesión 02 mostró que para transmitir $k$ bits por símbolo con M-QAM se necesita un SNR proporcional a $(M-1)$. La Sesión 02 resolvió el canal AWGN plano — un coeficiente escalar de canal que el receptor puede invertir directamente. Aquí el canal es frequency-selective — no existe un único coeficiente que corrija todo el espectro. Pero hay un problema que no resolvimos: ¿qué ocurre cuando aplicamos una única portadora M-QAM de alta tasa sobre un canal frequency-selective?
...
El mismo fenómeno visto desde el dominio de la frecuencia revela el problema de fondo. Los ecos llegan con retardos distintos y se suman vectorialmente: en algunas frecuencias se refuerzan (interferencia constructiva), en otras se cancelan (interferencia destructiva). El resultado es $H(f)$ no uniforme — la Sesión 01 denominó *deep fade* a los valles donde la ganancia cae casi a cero. Un símbolo de banda ancha $B$ experimenta simultáneamente frecuencias con ganancia alta y frecuencias en *deep fade*: no existe un único coeficiente de ecualización que corrija todo el espectro a la vez. El problema no es solo ISI en tiempo — es que el canal impone una ganancia diferente a cada frecuencia, y cualquier intento de ecualización global con un solo coeficiente es una aproximación.
