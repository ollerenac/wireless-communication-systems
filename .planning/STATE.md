...
- R4 se ADOPTA, no se deriva (2026-08-12): toda derivación de throughput de borde desde la demanda es inválida por (i) circularidad — necesita el despliegue real, que es resultado de Fases 2-4 — y (ii) incoherencia de verificación — el mapa F6 es mono-usuario, un requisito "bajo carga" no es lo que se mide. Planteamiento vigente: piso = servicio prometido (Tabla 0.3) → ancla publicada (NGMN 50 / ITU-R 100) → adopción declarada. UL = columna UL de Tabla 0.3 sobre servicios prometidos. R5 gobierna la carga (sí derivable); R4 la calidad por usuario. Dos intentos de fórmula (N_borde, processor sharing) fueron removidos por esto.
